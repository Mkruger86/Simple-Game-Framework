    ...
    public class World
    {
        ...
        public void AddWorldObjectWithItem(WorldObject worldObject, Item? item, Vector2 position)
        {
            if (IsWithinBounds(position))
            {
                ...
                if (item != null)
                {
                    _worldObjectItems[worldObject] = item; // Associate the item with the WorldObject
                    item.SetWorldObject(worldObject); // Set the WorldObject reference in the item
        ...
        public void RenderGrid(Player player)
        {
            ...
            for (int y = 0; y < Height; y++)
            {
                ...
                for (int x = 0; x < Width; x++)
                {
                    ...
                    {
                        Console.Write("x "); // Player's position
                    }
                    else if (CreaturePositions.Values.Any(p => p == position))
                    {
                        Console.Write("C "); // Creature's position
                    }
                    else if (WorldObjectPositions.Values.Any(p => p == position))
                    {
                        Console.Write("i "); // WorldObject's position
                    }
                    else
                    {
                        var node = Graph.GetNode(x, y);
                        Console.Write(node.Walkable ? ". " : "# "); // Walkable or non-walkable node
                    }
                }
                Console.WriteLine("|");
            }
